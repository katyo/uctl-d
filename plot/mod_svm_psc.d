...
nothrow @nogc void entry() {
  auto osc_param = mk!(OSC.Param, qrev, dt)(freq.as!Hz);
  auto osc_state = OSC.State!(osc_param, float)();

  auto psc_param = mk!(PSC.Param)(tcrit.as!sec, mfreq.as!Hz);
  ...
  foreach (i; 0 .. cast(int)(tend/dt)) {
    float t = i * dt;
    auto phase = osc_state(osc_param);
    ...
    auto abc3 = abc2.scale!(0.0, 1.0, -1.0, 1.0);
